...
return function()
  ...
  local custom = h.new_buffer('<div data-entry-custom="$custom"></div>', "jinja")
  local unsupported = h.new_buffer('<div data-show="$lua"></div>', "lua")

  local datastar = require("datastar")
  datastar.setup({ custom_attributes = { "entry-custom" } })
  ...
  h.assert_true(#h.extmarks(custom) > 0, "explicit options apply to pre-existing expanded filetypes")
  ...
  h.assert_equal({ "entry-custom" }, datastar._state().config.custom_attributes, "loading after explicit setup preserves configuration")
  h.assert_true(#h.extmarks(custom) > 0, "loading after explicit setup preserves custom marks")
  h.assert_equal(before_loader, h.autocmd_topology("datastar.nvim"), "loading after explicit setup does not duplicate autocmds")

  datastar.setup()
  h.assert_equal({ "entry-custom" }, datastar._state().config.custom_attributes, "no-argument setup preserves explicit configuration")
